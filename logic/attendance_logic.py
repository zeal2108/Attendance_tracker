...
def mark_entry():
    ...
    existing = database.get_today_log(today)

    if existing is None:
        database.insert_entry(today, current_time)
    else:
...
def mark_exit():
    ...
    if existing and existing[1] is None:
        database.update_exit(today, current_time)
    else:
...
def get_today_log():
    today = get_today_date()
    log = database.get_today_log(today)
    if log:
